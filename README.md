...
$$P(V\_X) = \mathcal{N}(\mu\_X, \sigma^2\_X)$$
...
$$\sigma^2\_\text{unattended}(t+1) = \sigma^2\_\text{unattended}(t) + \alpha$$
...
$$z = \frac{-|\mu\_L - \mu\_R|}{\Delta\sigma}, \quad \Delta\sigma = \sigma\_{\text{unattended}} - \sigma\_{\min}$$
...
$$\text{VOI}\_{\text{switch}} + \epsilon > C\_{\text{shift}} + b\_{\text{pref}} \cdot (\mu\_{\text{attended}} - \mu\_{\text{unattended}})$$
...
$$v\_X \sim \mathcal{N}(\mu\_X, \sigma^2\_X + \sigma^2\_{\text{decision}}), \quad X \in \{L, R\}$$

$$\text{Choice} = \arg\max(v\_L, v\_R)$$
...
| Observation noise | `sigma2_obs` | 0.2 | Kalman filter observation noise variance |
| Initial uncertainty | `sigma2_init` | 4.0 | Starting uncertainty for each option |
| Decision noise | `sigma2_decision` | 0.02 | Additional noise at the final choice stage |
